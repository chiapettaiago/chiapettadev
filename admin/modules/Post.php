...
class Post {
    ...
    public static function create($data, $userId) {
        try {
            ...
            if (($postData['status'] ?? 'draft') === 'published') {
    ...
    public static function update($postId, $data, $userId) {
        try {
            ...
            if ($post['slug'] !== $slug) {
                self::removePublicPage($post['slug']);
            }

            if (($updateData['status'] ?? 'draft') === 'published') {
                self::syncPublicPage($slug);
            } else {
                self::removePublicPage($slug);
            }
    ...
    /**
     * Sincronizar página pública do post
     */
    private static function syncPublicPage($slug) {
        ...
        $dir = __DIR__ . '/../../blog/' . $slug;
        if (!is_dir($dir)) {
            mkdir($dir, 0755, true);
        }

        $index = $dir . '/index.php';
        $content = "<?php\n// Generated by admin/modules/Post.php\n\$blogPostSlug = '" . addslashes($slug) . "';\nrequire __DIR__ . '/../../templates/blog-post.php';\n";
        file_put_contents($index, $content);
    }

    /**
     * Remover página pública gerada do post
     */
    private static function removePublicPage($slug) {
        ...
        $index = __DIR__ . '/../../blog/' . $slug . '/index.php';
    ...
    /**
     * Normalizar slug para caminho público
     */
